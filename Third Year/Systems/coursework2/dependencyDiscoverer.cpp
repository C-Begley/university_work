...
//std::unordered_miap<std::string, std::list<std::string>> theTable;
//std::list<std::string> workQ;
int CRAWLER_THREADS = 1;
...
struct workQStruct {
    ...
    public :
        ...
        //Returns the first item in queue or waits until item in work queue
        std::string pop_front() {
            std::unique_lock<std::mutex> lock(m);
            ready.wait(lock, [this]{ return (!Q.empty()) ||done;}); //wait until item in work queue or told to end
            if(Q.empty()) return "NULL";  //If the list is empty return exit codition

            //Else pop and return the first item in Queue
...
struct tableStruct {
    ...
    public : 
        ...
        void insertFirst(std::string name){
            std::unique_lock<std::mutex> lock(m);
        ...
        bool notInTable(std::string filename) {
            return Table.find(filename) == Table.end();
        }
...
int workThreadProcess() {
  auto filename = workQ.pop_front();
  while (filename != "NULL") {

  if (theTable.notInTable(filename)) {
