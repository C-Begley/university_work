...
int run_driver(CLObject* ocl,unsigned int buffer_size,  int* input_buffer_1, int* input_buffer_2, int* output_buffer) {
    long long unsigned int tid = ocl->thread_num;
     ...
     global = buffer_size; // cre0,ate as meany threads on the device as there are elements in the array
...
// START of assignment code section 
    ...
    for(int i = 0; i<max_iters; i++){
        ...
        //Output buffers
        output = clCreateBuffer(ocl->context,CL_MEM_READ_WRITE, buffer_size,output_buffer,&err);

        status_buf = clCreateBuffer(ocl->context,CL_MEM_READ_WRITE,sizeof(int),&status, &err);

        // Write the data in input arrays into the device memory 
      
        //[YOUR CODE HERE]
        clEnqueueWriteBuffer(ocl->command_queue,input1,CL_TRUE,0,buffer_size,input_buffer_1,0,NULL,NULL);

        clEnqueueWriteBuffer(ocl->command_queue,input2,CL_TRUE,0,buffer_size,input_buffer_2,0,NULL,NULL);

        // Set the arguments to our compute kernel
    
        //[YOUR CODE HERE]
        clSetKernelArg(ocl->kernel,0,sizeof(&input1),&input1);
        clSetKernelArg(ocl->kernel,1,sizeof(&input2),&input2);
        clSetKernelArg(ocl->kernel,2,sizeof(&output),&output);
        clSetKernelArg(ocl->kernel,3,sizeof(&status_buf),&status_buf);
        clSetKernelArg(ocl->kernel,4,sizeof(&buffer_size), &buffer_size);
        ...
        // Check the status
        clEnqueueReadBuffer(ocl->command_queue,status_buf,CL_TRUE,0,sizeof(int), &status, 0,NULL,NULL); 
    ...
    if (status == 0){ 
        clEnqueueReadBuffer(ocl->command_queue,output,CL_TRUE,0,buffer_size, &output_buffer,0,NULL,NULL);
        // Shutdown and cleanup 
    
        //[YOUR CODE HERE]clReleaseMemObject(input1);
