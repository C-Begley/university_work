...
int q_add(Queue *q, Item i, int prio) {
	...
	if (p != NULL) {
        ...
        while(current-> next != NULL){ //Haven't reached end of list
            if (current->next->prio > p->prio){  //If p has a higher priorirty then next element, insert p here
                p->next=current->next;
                current->next=p;
                return 1;    
