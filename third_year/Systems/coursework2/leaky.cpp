...
struct tableStruct {
    ...
    public : 
        ...
        //Functions set to mirror unordered_map functions, but be thread safe  
        ...
        void insert (std::pair<std::string, std::list<std::string>> list ){
             std::unique_lock<std::mutex> lock (m);
             Table.insert(list);
        }
        //Returns pointer to unordered_map key
        std::list<std::string> * get(std::string name) {
            std::unique_lock<std::mutex> lock (m);
            return &Table[name];
        }


}; typedef tableStruct tableStruct;

//Thread safe structure for work queue
struct workQStruct {
    private :   
    ...
    public :
        ...
        //Functions that mirror list functions, but are thread protected
        void push_back(std::string string){
            std::unique_lock<std::mutex> lock (m);
            Q.push_back(string);         
        }

        std::unordered_map<std::string, std::list<std::string>>::size_type size() {
            std::unique_lock<std::mutex> lock(m);
            return Q.size();
        }
        ...
        // Returns the front of the queue or an empty string
        std::string front() {
            std::unique_lock<std::mutex> lock(m);
            std::string toReturn;
            if (!Q.empty()) {
        ...
        //Checks if the thread is still ready to work i.e the workQueue is not empty
        int ready() {
            std::unique_lock<std::mutex> lock(m);
            waitingThreads ++;
            waiting.wait(lock,[this]() {return Q.size() !=0 || waitingThreads == CRAWLER_THREADS;});
            if (Q.size()!= 0) {
                waitingThreads --;
                return 1;
            }
...
}; typedef workQStruct workQStruct;

tableStruct theTable;
workQStruct workQ;
...
//Function worker threads carry out
void workThreadProcess(){
    int running = 1; //Boolean to show threads are running
    while(running) {

        running = workQ.ready();//Check if work to do
        std::string filename = workQ.front();//Get file item in work queue
     
        if(!filename.empty() ) { //If work queue wasn't emptied between checking and getting file
            //If file was last edited
            if (theTable.find(filename) == theTable.end()) {
                fprintf(stderr, "Mismatch between table and workQ\n");
            }
            //If file valid, process it
            else {
